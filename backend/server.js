...
// Moved server-side from src/services/founderAccess.ts's old
// EXPO_PUBLIC_-prefixed versions, which were readable in plain text by
// anyone who decompiled the client bundle. These three are server-only
// env vars now (never EXPO_PUBLIC_*, never referenced from src/) --
// keep them in backend/.env locally and as Vercel project env vars
// only. See POST /v1/founder-access/redeem below.
// No built-in defaults: production ran on guessable fallbacks
// ("JESUSFOUNDER" and friends) while these env vars were unset, which
// handed free Platinum to anyone who tried them. An unset code simply
// can't be redeemed (see the 503 in POST /v1/founder-access/redeem).
const readAccessCode = (name) => {
  const value = process.env[name]?.trim().toUpperCase();
  if (value && value.length < 12) console.warn(`[security] ${name} is under 12 characters -- use a longer, random code.`);
  return value || null;
};
const FOUNDER_ACCESS_CODE = readAccessCode('FOUNDER_ACCESS_CODE');
const FAMILY_ACCESS_CODE = readAccessCode('FAMILY_ACCESS_CODE');
const DEV_ACCESS_CODE = readAccessCode('DEV_ACCESS_CODE');
...
// Same timing-safe-compare reasoning as isDeveloperToken above, applied
// to the three founder-access codes -- see POST /v1/founder-access/redeem.
function timingSafeCodeEquals(provided, expected) {
...
// Rendered from legal-documents.json -- the same file the app shows
// during onboarding (see the require just below renderLegalPage).
...
// All three documents live in legal-documents.json, the single source of
// truth shared with the app (src/constants/legal.ts imports the same
// file), so these public pages can never drift from the text users
// accept in onboarding.
const {
  aiDisclosure: AI_DISCLOSURE,
  termsOfService: USER_AGREEMENT,
  privacyPolicy: PRIVACY_POLICY,
} = require('./legal-documents.json');
...
const ACCOUNT_DELETION = {
  ...
  intro:
    'Jesus Interactive (published by Jesus Interactive Inc.) lets you permanently delete your account and all of your data directly within the app -- no separate request, email, or waiting period required.',
  sections: [
    {
...
const MANAGE_DATA = {
  ...
  lastUpdated: 'September 11, 2026',
  intro:
    'Jesus Interactive (published by Jesus Interactive Inc.) lets you delete specific pieces of your data without deleting your entire account. Each option below is immediate and permanent -- there is no waiting period.',
  sections: [
    {
...
// network.
// Catch-all for authenticated routes that don't need a tighter,
// purpose-specific limiter of their own (feed reads, export, account
// deletion) -- so no route is left completely unthrottled.
const generalLimiter = rateLimit({
  windowMs: 60 * 1000,
  limit: Number(process.env.GENERAL_RATE_LIMIT) || 60,
  standardHeaders: true,
  legacyHeaders: false,
  message: { error: 'Too many requests -- please try again shortly.' },
  skip: isDeveloperRequest,
});
// Admin routes. Unlike every other limiter this one does NOT skip for the
// developer token -- it's the cap on guessing at that token.
const adminLimiter = rateLimit({
  windowMs: 15 * 60 * 1000,
  limit: Number(process.env.ADMIN_RATE_LIMIT) || 100,
  standardHeaders: true,
  legacyHeaders: false,
...
// Tight on purpose -- these are short, shared, guessable-shape codes
// (see FOUNDER_ACCESS_CODE etc. below), so the rate limit is the main
// thing standing between "the codes moved server-side" and "someone
// just brute-forces the endpoint instead of decompiling the bundle."
const founderRedeemLimiter = rateLimit({
  windowMs: 15 * 60 * 1000,
  limit: Number(process.env.FOUNDER_REDEEM_RATE_LIMIT) || 5,
  standardHeaders: true,
  legacyHeaders: false,
...
app.delete('/v1/account', generalLimiter, requireAuth, async (req, res) => {
...
app.post('/v1/account/export', generalLimiter, requireAuth, (req, res) => {
  res.status(501).json({
    error:
...
// Server-side home for the founder/family/dev free-Platinum codes that
// used to live in src/services/founderAccess.ts as EXPO_PUBLIC_ values
// (readable in plain text by decompiling the client bundle). The code
// itself is checked here now, never in the client, and this is also
// the only place that authoritatively grants the plan -- unlike
// /v1/device/heartbeat above (which never trusts the client's plan as
// an entitlement), this route's own `users` write IS the entitlement
// grant, since a correct code is the whole point of the check.
// Deliberately grants lifetime Platinum (no plan_expires_at) -- these
// codes were always meant as a permanent comp, not a trial.
app.post('/v1/founder-access/redeem', founderRedeemLimiter, requireAuth, requireDatabase, async (req, res) => {
  const { deviceId, code } = req.body || {};
  if (!deviceId || typeof deviceId !== 'string' || deviceId.length > 200) {
    return res.status(400).json({ error: 'deviceId is required' });
  }
  if (!code || typeof code !== 'string' || code.length > 60) {
    return res.status(400).json({ error: 'Invalid code' });
  }
  const normalized = code.trim().toUpperCase();
  if (!FOUNDER_ACCESS_CODE && !FAMILY_ACCESS_CODE && !DEV_ACCESS_CODE) {
    return res.status(503).json({ error: 'Code redemption is not available right now.' });
  }
  // Check all three regardless of an early match, so response timing
  // can't reveal which (if any) code family a guess was close to.
  const matchesFounder = timingSafeCodeEquals(normalized, FOUNDER_ACCESS_CODE);
  const matchesFamily = timingSafeCodeEquals(normalized, FAMILY_ACCESS_CODE);
  const matchesDev = timingSafeCodeEquals(normalized, DEV_ACCESS_CODE);
  const kind = matchesFounder ? 'founder' : matchesFamily ? 'family' : matchesDev ? 'dev' : null;
  if (!kind) {
    // 403, not 401 -- this is a valid, authenticated request with a
    // wrong code, not a bad/expired session token. Using 401 here would
    // also wrongly trigger the client's withAuthRetry 401-retry logic.
    return res.status(403).json({ error: 'Invalid code' });
  }
  try {
    await sql`
      INSERT INTO users (device_id, plan, plan_expires_at, last_seen_at)
      VALUES (${deviceId}, 'platinum', NULL, now())
      ON CONFLICT (device_id) DO UPDATE
        SET plan = 'platinum', plan_expires_at = NULL, last_seen_at = now()
    `;
    await sql`INSERT INTO subscription_events (device_id, plan, event_type) VALUES (${deviceId}, 'platinum', ${'founder_redeem_' + kind})`;
    // Audit trail only -- never logs the code value itself, just which
    // family of code was used and which device redeemed it.
    console.warn(`[audit] founder-access redeemed: kind=${kind} deviceId=${deviceId}`);
    res.status(200).json({ ok: true, plan: 'platinum', kind });
  } catch (err) {
    console.error('[founder-access/redeem] failed:', err);
...
app.get('/v1/testimonies/stats', generalLimiter, requireAuth, requireDatabase, async (req, res) => {
  try {
...
app.get('/v1/testimonies', generalLimiter, requireAuth, requireDatabase, async (req, res) => {
...
app.get('/v1/trivia/leaderboard', generalLimiter, requireAuth, requireDatabase, async (req, res) => {
...
app.get('/v1/admin/stats', adminLimiter, requireAuth, requireDeveloper, requireDatabase, async (req, res) => {
  try {
...
app.get('/v1/admin/testimonies', adminLimiter, requireAuth, requireDeveloper, requireDatabase, async (req, res) => {
...
app.post('/v1/admin/testimonies/:id/status', adminLimiter, requireAuth, requireDeveloper, requireDatabase, async (req, res) => {
  const { id } = req.params;
...
app.post('/v1/admin/users/:deviceId/flag', adminLimiter, requireAuth, requireDeveloper, requireDatabase, async (req, res) => {
...
app.post('/v1/admin/trivia/questions/bulk', adminLimiter, requireAuth, requireDeveloper, requireDatabase, async (req, res) => {
...
// directly.
app.post('/v1/admin/trivia/questions/:id', adminLimiter, requireAuth, requireDeveloper, requireDatabase, async (req, res) => {
  const { id } = req.params;
...
app.post('/v1/admin/trivia/questions/:id/status', adminLimiter, requireAuth, requireDeveloper, requireDatabase, async (req, res) => {
  const { id } = req.params;
...
app.post('/v1/admin/radio/config', adminLimiter, requireAuth, requireDeveloper, requireDatabase, async (req, res) => {
...
app.post('/v1/admin/emergency-directory', adminLimiter, requireAuth, requireDeveloper, requireDatabase, async (req, res) => {
